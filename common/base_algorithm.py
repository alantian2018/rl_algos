...
class BaseAlgorithm:
    def __init__(
        ...
        self.obs_dim = self.raw_obs_dim[:-1] + (
            self.raw_obs_dim[-1] * self.config.frame_stack,
        )
    ...
    def load_ckpt_if_needed(self):
        if self.config.path_to_checkpoint:
            ...
            networks = self.logger.load_checkpoint(
                path=path, networks=self._get_networks()
            )
    ...
    def save_ckpt(self, step: int):
        ...
        if self.config.save_freq is not None:
            self.logger.save_checkpoint(networks=self._get_networks(), step=step)
