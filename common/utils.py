...
class Logger:
    ...
    def log_rollout(self, episode_returns: list, step: int):
        ...
        if self.use_wandb and episode_returns:
            self.log(
                {
                    "rollout/episode_return_mean": sum(episode_returns)
                    / len(episode_returns),
    ...
    def load_checkpoint(self, path: str, networks: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(
            termcolor.colored(
                f"Loaded checkpoint from {path} (step {checkpoint['step']})", "green"
            )
        )
        return networks
