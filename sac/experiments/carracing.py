...
def make_carracing_env(render_mode=None, normalize=True):
    env = gymnasium.make("CarRacing-v3", continuous=True, render_mode=render_mode,)
    if not normalize:
        return env
...
def main(config: CarRacing):
    ...
    policy_encoder = CNNEncoder(in_channels=in_channels, height=height, width=width, hidden_dim = config.hidden_dim)
    qf1_encoder = CNNEncoder(in_channels=in_channels, height=height, width=width, hidden_dim = config.hidden_dim)
    qf2_encoder = CNNEncoder(in_channels=in_channels, height=height, width=width, hidden_dim = config.hidden_dim)

    policy = Policy(encoder=policy_encoder, action_dim=config.action_dim, action_low=config.action_low, action_high=config.action_high)
